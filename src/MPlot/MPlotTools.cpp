...
void MPlotDragTool::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if(event->button() == Qt::LeftButton) {
        dragInProgress_ = true;
        this->grabMouse();
...
void MPlotDragTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    // Possible transition: A drag event has started, and the user exceeded the drag deadzone to count as a real drag.
    if(dragInProgress_ ) {
        double offsetMouse = ( event->pos().x()-_prevX);
        // if we've gone far enough, this counts as a real drag:
        if(std::abs(offsetMouse) > MPLOT_RUBBERBAND_DEADZONE) {
            // Disable auto-scaling on the plot... the user probably wants to take over manual control... (We don't want the plot jumping while they're auto-scaling it.)
            foreach(MPlotAxisScale* axis, targetAxes_) {
                if(axis->orientation() == Qt::Horizontal) {
                    double width = event->widget()->width();
                    double offset =-(offsetMouse/width)*(axis->dataRange().max()-axis->dataRange().min());
                    axis->setDataRange(MPlotAxisRange(
                                           axis->dataRange().min() + offset,
                                           axis->dataRange().max() + offset
                                           ),false
                                       );
                }
            }
...
void MPlotDragTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    // left mouse button release: drag event is done.
    if(event->button() == Qt::LeftButton) {
...
void MPlotDragTool::wheelEvent(QGraphicsSceneWheelEvent *event)
{
    QGraphicsObject::wheelEvent(event);
}

void MPlotDragTool::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    QGraphicsObject::mouseDoubleClickEvent(event);
}
